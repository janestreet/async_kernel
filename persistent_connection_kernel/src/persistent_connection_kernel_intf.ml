...
module type S = sig
  ...
  *)
  ...
  (** [create ~server_name ~on_event ~retry_delay get_address] returns a persistent
      ...
      [const (sec 10.)]. Note that what this retry delay actually throttles is the
      delay between two connection attempts, so when a long-lived connection dies,
      connection is usually immediately retried, and if that failed, wait for another
      retry delay and retry.
