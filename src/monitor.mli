(** The part of the {{!Async_kernel.Execution_context} [Execution_context]} that
    determines what to do when there is an unhandled exception.
    ...
    exception to no longer propagate to the parent monitor. Then whoever detached it is
    responsible for dealing with the exception. If the monitor is still attached, then the
    exception bubbles to monitor's parent. If an exception bubbles to the root of the
    monitor tree, that prints an unhandled-exception message and calls exit 1.
...
(** [detach t] detaches [t], removing its parent, so that errors raised to [t] are not
    passed to its former parent monitor. If those errors aren't handled in some other way
    (e.g. via [get_next_error]), then they will be ignored. One should usually use
...
module Monitor_exn : sig
  ...
  (** Extract the backtrace originally captured at the point the exception was received by
      a monitor. *)
  val backtrace : t -> Backtrace.t option
end

(** When [extract_exn] is set to false, exceptions returned by [try_with] are wrapped into
    this exception type which also provides backtrace information. *)
...
(** [extract_exn exn] extracts the exn from an error exn that comes from a monitor. If it
    is not supplied such an error exn, it returns the exn itself. It removes the backtrace
...
(** [try_with_or_error] is like [try_with] but returns ['a Or_error.t Deferred.t] instead
    ...
    {[
      ...
      = try_with f ?extract_exn ~run:`Now ~rest
