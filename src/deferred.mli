...
(** [enabled [choice t1 f1; ... choice tn fn;]] returns a deferred [d] that becomes
    determined when any of the [ti] becomes determined. The value of [d] is a function [f]
...
(** {[
    ...
    of the [fi]s, the one that determines its result. There is no guarantee that the [ti]
    that becomes determined earliest in time will be the one whose value determines the
    [choose]. Nor is it guaranteed that the value in [t] is the first value (in place
    order) from [choices] that is determined at the time [t] is examined.

    For example, in:

    {[
      choose [ choice t1 (fun () -> `X1); choice t2 (fun () -> `X2) ]
      >>> function
      | `X1 -> e1
      | `X2 -> e2
    ]}
...
(** [repeat_until_finished initial_state f] repeatedly runs [f] until [f] returns
    [`Finished]. The first call to [f] happens immediately when [repeat_until_finished] is
    called. *)
...
(** {2 Deferred collections}
    ...
    These contain operations for iterating in a deferred manner over different collection
    types. *)
