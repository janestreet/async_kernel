...
let set (type a : value_or_null) t i execution_context f a =
...
let enqueue (type a : value_or_null) t execution_context f (a : a) =
...
let run_jobs (type a) t scheduler =
  ...
  (* [run_external_jobs] before entering the loop, since it might enqueue a job, changing
     [t.length]. *)
  try
