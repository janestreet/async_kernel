...
type ('a : value_or_null) t = 'a Types.Ivar.t = { mutable cell : ('a, any) cell }
...
and ('a : value_or_null, 'b) cell = ('a, 'b) Types.Cell.t =
  | Empty_one_or_more_handlers :
      ('a : value_or_null).
  ...
  | Empty : ('a : value_or_null). ('a, [> `Empty ]) cell
  | Full : ('a : value_or_null). 'a -> ('a, [> `Full ]) cell
  | Indir : ('a : value_or_null). 'a t -> ('a, [> `Indir ]) cell

module Handler = struct
  type ('a : value_or_null) t = ('a, [ `Empty_one_or_more_handlers ]) cell
  ...
  let create (type a : value_or_null) (run : a -> unit) execution_context : a t =
  ...
  let add (type a : value_or_null) (t : a t) (run : a -> unit) execution_context =
    let result =
...
let squash =
  let rec follow : ('a : value_or_null). ('a, any) cell -> 'a ivar -> ('a, any) cell =
    fun indir t ->
    (* [indir = Indir t] *)
    match t.cell with
    ...
    | _ -> indir
  in
  let rec update : ('a : value_or_null). 'a ivar -> ('a, any) cell -> 'a ivar =
    fun t indir ->
    match t.cell with
    | Indir t' ->
      t.cell <- indir;
...
let sexp_of_t (type a : value_or_null) (sexp_of_a : a -> Sexp.t) (t : a ivar) : Sexp.t =
  let t = squash t in
  match t.cell with
  | Indir _ -> assert false (* fulfilled by [squash] *)
...
let fill_exn (type a : value_or_null) (t : a t) (v : a) =
  let t = squash t in
  match t.cell with
  | Indir _ -> assert false (* fulfilled by [squash] *)
...
let remove_handler (type a : value_or_null) (t : a t) (handler : a Handler.t) =
...
let add_handler (type a : value_or_null) (t : a t) run execution_context =
  let t = squash t in
  match t.cell with
  | Indir _ -> assert false (* fulfilled by [squash] *)
  | Empty ->
...
let has_handlers (type a : value_or_null) (t : a t) =
  let t = squash t in
  match t.cell with
  | Indir _ -> assert false (* fulfilled by [squash] *)
...
let upon' (type a : value_or_null) (t : a t) run =
